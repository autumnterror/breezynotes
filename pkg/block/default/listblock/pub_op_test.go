...
func TestOp(t *testing.T) {
	...
	t.Run("apply style", func(t *testing.T) {
		...
		if assert.NoError(t, err) {
			...
			if assert.NoError(t, err) {
				block.Data = s
				lst, err := domain.FromUnifiedToListBlock(block)
				if assert.NoError(t, err) {
	...
	t.Run("change_type", func(t *testing.T) {
		t.Run(domain.ListBlockToDoType, func(t *testing.T) {
			block := testBlockTodo()
			lstOld, err := domain.FromUnifiedToListBlock(block)
			assert.NoError(t, err)

			data, err := d.Op(ctx, block, "change_type", map[string]any{
				"new_type": domain.ListBlockToDoType,
			})
			if assert.NoError(t, err) {
				s, err := structpb.NewStruct(data)
				if assert.NoError(t, err) {
					...
					lst, err := domain.FromUnifiedToListBlock(block)
					if assert.NoError(t, err) {
						assert.Equal(t, domain.ListBlockToDoType, lst.Data.Type)
						...
						log.Println(domain.ListBlockToDoType, format.Struct(block))
					}
				}
			}
		})
		t.Run(domain.ListBlockOrderedType, func(t *testing.T) {
			...
			data, err := d.Op(ctx, block, "change_type", map[string]any{
				"new_type":      domain.ListBlockOrderedType,
				"ordered_value": 2,
			})
			if assert.NoError(t, err) {
				s, err := structpb.NewStruct(data)
				if assert.NoError(t, err) {
					...
					lst, err := domain.FromUnifiedToListBlock(block)
					if assert.NoError(t, err) {
						assert.Equal(t, domain.ListBlockOrderedType, lst.Data.Type)
						assert.Equal(t, lstOld.Data.TextData, lst.Data.TextData)
						assert.Equal(t, 2, lst.Data.Value)
						assert.Equal(t, lstOld.Data.Level, lst.Data.Level)
						log.Println(domain.ListBlockOrderedType, format.Struct(block))
					}
				}
			}
		})
		t.Run(domain.ListBlockUnorderedType, func(t *testing.T) {
			...
			data, err := d.Op(ctx, block, "change_type", map[string]any{
				"new_type": domain.ListBlockUnorderedType,
			})
			if assert.NoError(t, err) {
				s, err := structpb.NewStruct(data)
				if assert.NoError(t, err) {
					...
					lst, err := domain.FromUnifiedToListBlock(block)
					if assert.NoError(t, err) {
						assert.Equal(t, domain.ListBlockUnorderedType, lst.Data.Type)
						assert.Equal(t, lstOld.Data.TextData, lst.Data.TextData)
						assert.Equal(t, 0, lst.Data.Value)
						assert.Equal(t, lstOld.Data.Level, lst.Data.Level)
						log.Println(domain.ListBlockUnorderedType, format.Struct(block))
					}
				}
			}
		})
...
func TestChangeTypeFromList(t *testing.T) {
	...
	t.Run(domain.TextBlockType, func(t *testing.T) {
		block := testBlock()
		list, _ := domain.FromUnifiedToListBlock(block)
