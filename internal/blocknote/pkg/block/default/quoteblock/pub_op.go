...
// GetAsFirst returns the text content of the quote.
func (d *Driver) GetAsFirst(ctx context.Context, block *brzrpc.Block) string {
	b, err := domainblocks.FromUnifiedToQuoteBlock(block)
...
// ChangeType converts the QuoteBlock to another block type.
func (d *Driver) ChangeType(ctx context.Context, block *brzrpc.Block, newType string) error {
	const op = "quoteblock.ChangeType"
	b, err := domainblocks.FromUnifiedToQuoteBlock(block)
	if err != nil {
