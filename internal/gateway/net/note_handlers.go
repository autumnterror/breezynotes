...
// @Produce json
// @Param id query string true  "Note ID"
// @Success 200 {object} []domain.UserWithRole
// @Failure 401 {object} domain.Error
// @Failure 502 {object} domain.Error
// @Failure 504 {object} domain.Error
...
func (e *Echo) GetRoles(c echo.Context) error {
	...
	var res []domain.UserWithRole

	for _, info := range infos.Users {
		switch {
		case info.Id == n.Author:
			res = append(res, *domain.UserWithRoleFromRpc(info, "author"))
		case alg.IsIn(info.Id, n.Editors):
			res = append(res, *domain.UserWithRoleFromRpc(info, "editor"))
		case alg.IsIn(info.Id, n.Readers):
			res = append(res, *domain.UserWithRoleFromRpc(info, "reader"))
		}
	}

	sort.Slice(res, func(i, j int) bool {
		return res[i].Login < res[j].Login
	})
