...
**Внимание!** Временно некоторые ошибки могут возвращаться в расширенном формате `Error Trace`. В этом случае валидацию сообщения следует проводить по частичному совпадению строки.
*Пример:* `op: grpc.Auth Error: ... : email and login is empty` вместо `"email and login is empty"`.
